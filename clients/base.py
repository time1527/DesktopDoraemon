...
class BaseClient(QWidget):
    ...
    def init_ui(self) -> None:
        ...
        # 初始化时展示一些对话
        bubble_message = BubbleMessage('你好，我是哆啦A梦！', 
                                       self.receive_avatar, 
                                       type = MessageType.TEXT,
                                       role = MessageRole.SYSTEM)
        self.msg_show_frame.add_message_item(bubble_message)

        bubble_message = BubbleMessage(os.path.join(REPO_PATH,'data/ui/chat/head1.jpg'), 
                                       self.receive_avatar, 
        ...
        # 设置可拉伸
        self.msg_input_frame.setSizePolicy(QSizePolicy.Expanding, 
        ...
        button_height = self.msg_send_button.sizeHint().height() + \
    ...
    def send_msg(self):
        ...
        bubble_message = BubbleMessage(msg, 
                                       self.send_avatar, 
                                       type = MessageType.TEXT, 
    ...
    def open_file(self):
        ...
        if is_image(path[0]):
            bubble_message = BubbleMessage(path[0], 
                                           self.send_avatar, 
                                           type = MessageType.IMAGE, 
                                           role = MessageRole.USER)
            self.msg_show_frame.add_message_item(bubble_message)
    ...
    def message_to_bubble_messages(self,message: Message) -> Union[List[BubbleMessage],BubbleMessage]:
        ...
        if isinstance(content,str): # only text
            bmsgs.append(BubbleMessage(content, 
                                       avatar,
                                       type = MessageType.TEXT,
                                       role = role)) 
