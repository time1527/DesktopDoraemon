...
from utils.schema import ContentItem, Message,MessageRole,MessageType,DEFAULT_SYSTEM_PROMPT
...
def format_as_multimodal_message(
    ...
    assert msg.role in (MessageRole.USER, 
    ...
    if isinstance(msg.content, str):  # if text content
        ...
        for item in msg.content:
            ...
            if k in ('file', 'image'):
                ...
                files.append(v)
        if add_upload_info and files and (msg.role in (MessageRole.SYSTEM, MessageRole.USER)):
            if lang == 'auto':
...
def has_chinese_messages(messages: List[Union[Message, dict]], check_roles: Tuple[str] = (MessageRole.SYSTEM, MessageRole.USER)) -> bool:
    for m in messages:
...
def build_text_completion_prompt(
    ...
    for msg in messages:
        ...
        content = msg.content
        if allow_special:
            assert msg.role in (MessageRole.USER, MessageRole.ASSISTANT, MessageRole.SYSTEM, MessageRole.FUNCTION)
...
def get_file_type(path: str) -> Literal['json','jsonl','md','pdf', 'docx', 'pptx', 'txt', 'html', 'csv', 'tsv', 'xlsx', 'xls', 'unk']:
    f_type = get_basename_from_url(path).split('.')[-1].lower()
    if f_type in ['json','jsonl','md','pdf', 'docx', 'pptx', 'txt','csv', 'tsv', 'xlsx', 'xls']:
