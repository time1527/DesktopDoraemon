...
class BaseFnCallModel(BaseChatModel, ABC):
    ...
    def __init__(self, cfg: Optional[Dict] = None):
        ...
        if fncall_prompt_type == 'qwen':
            from .fncall_prompts.qwen_fncall_prompt import FN_STOP_WORDS, QwenFnCallPrompt
    ...
    def _remove_fncall_messages(self, messages: List[Message], lang: Literal['en', 'zh']) -> List[Message]:
        ...
        for msg in messages:
            if (msg.role == MessageRole.FUNCTION) or msg.function_call:
                ...
                if (msg.role == MessageRole.USER) and new_messages and (new_messages[-1].role == MessageRole.USER):
                    # Separate two user messages with an assistant message to make the bot focus on the latter:
                    new_messages.append(Message(role=MessageRole.ASSISTANT, content=[ContentItem(text='...')]))
