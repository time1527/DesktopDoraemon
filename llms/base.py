...
# https://github.com/QwenLM/Qwen-Agent/blob/main/qwen_agent/llm/base.py
...
from utils.schema import DEFAULT_SYSTEM_PROMPT, MessageRole, Message
from utils.tokenization_qwen import tokenizer
from utils.utils import (extract_text_from_message, 
...
from settings import DEFAULT_MAX_INPUT_TOKENS
# TODO：无论模型是什么，tokenizer都是qwen的吗？
...
class BaseChatModel(ABC):
    ...
    def quick_chat(self, prompt: str) -> str:
        """Quickly chat with the model."""
        *_, responses = self.chat(messages=[Message(role=MessageRole.USER, content=prompt)])
    ...
    def chat(
        ...
        # 合并generate_cfgs:
        generate_cfg = merge_generate_cfgs(base_generate_cfg = self.generate_cfg, 
        ...
        if messages[0].role != MessageRole.SYSTEM:
            messages = [Message(role=MessageRole.SYSTEM, content=DEFAULT_SYSTEM_PROMPT)] + messages

        # Not precise. It's hard to estimate tokens related with function calling and multimodal items.
        # 根据最大输入token数去截断messages:
        ...
        # {'image': 'G:\\DesktopDoraemon\\data\tools\\image\rm-img-bg-test.jpg'}]})]
        # ->
        # [Message({'role': 'system', 'content': 'You are a helpful assistant.'}), 
        # Message({'role': 'user', 'content': '（上传了 ![图片](imagem-img-bg-test.jpg)）\n\n你好'})]
        ...
        def _call_model_service():
            # fncall_mode = True -> self._chat_with_functions
            # fncall_mode = False -> 
            #                         最新消息来自ASSISTANT -> self._continue_assistant_response
            #                         最新消息没来自ASSISTANT -> self._chat
            if fncall_mode:
                ...
                if messages[-1].role == MessageRole.ASSISTANT:
                    ...
                    return self._continue_assistant_response(messages, 
                                                             generate_cfg=generate_cfg, 
                                                             stream=stream)
                else:
                    return self._chat(
        ...
        # 根据stream与delta_stream选择不同的对话函数:
        if stream and delta_stream:
        ...
        # -> format_as_text_message ?
        # -> convert_messages_to_target_type
    ...
    def _chat(
        ...
        """
        according to `stream`, chat with the model.

        Args:
            messages: Inputted messages.
            stream: Whether to use streaming generation.
            delta_stream: Whether to stream the response incrementally.
            generate_cfg: Extra LLM generation hyper-paramters.

        Returns:
            A list of response or a generator of response.
        """
        if stream:
    ...
    def _preprocess_messages(
        ...
        """Format the input messages to multimodal format."""
    ...
    def _postprocess_messages(
        ...
        """Format the output messages to multimodal format and postproc stop words."""
        messages = [format_as_multimodal_message(msg, add_upload_info=False) for msg in messages]
        # 如果generate_cfg中没有设置skip_stopword_postproc/设置为False，进行stopword的后处理:
...
def _truncate_input_messages_roughly(messages: List[Message], max_tokens: int) -> List[Message]:
    sys_msg = messages[0]
    assert sys_msg.role == MessageRole.SYSTEM  # The default system is prepended if none exists
