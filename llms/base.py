    ...
    def chat(
        ...
        if self.cache is not None:
            ...
            if cache_value:
                ...
                if stream:
                    cache_value: Iterator[List[Union[Message, dict]]] = iter(
                        [cache_value]
                    )
                return cache_value
        ...
        max_input_tokens = generate_cfg.pop(
        ...
        messages = self._preprocess_messages(
            messages, lang=lang, generate_cfg=generate_cfg, functions=functions
        )
        ...
        def _call_model_service():
            ...
            if fncall_mode:
                ...
                if messages[-1].role == MessageRole.ASSISTANT:
                    assert (
                        not delta_stream
                    ), "Continuation mode does not currently support `delta_stream`"
        ...
        if isinstance(output, list):
            ...
            if delta_stream:
                ...
                generate_cfg = copy.deepcopy(
                    generate_cfg
                )  # copy to avoid conflicts with `_call_model_service`
            ...
            return self._convert_messages_iterator_to_target_type(
                _format_and_cache(), _return_message_type
            )
...
def _truncate_input_messages_roughly(
    ...
    )  # The default system is prepended if none exists
