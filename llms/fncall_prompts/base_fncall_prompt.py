...
# https://github.com/QwenLM/Qwen-Agent/blob/main/qwen_agent/llm/fncall_prompts/base_fncall_prompt.py

from typing import List, Literal, Union

from utils.schema import MessageRole, Message
from utils.utils import format_as_multimodal_message, format_as_text_message, has_chinese_messages
    ...
    def format_plaintext_train_samples(
        ...
        if not parallel_function_calls:
            for i in range(len(messages) - 1):
                ...
                has_para = has_para or ((messages[i].role == MessageRole.FUNCTION) and (messages[i + 1].role == MessageRole.FUNCTION))
                if has_para:
