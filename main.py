...
from PyQt5.QtWidgets import QWidget, QApplication, QLabel, QVBoxLayout, QMenu,qApp, QDesktopWidget

from clients import ChatClient, ReActClient
...
class DesktopPet(QWidget):
    ...
    def init_pet(self):
        ...
        self.movie = QMovie(os.path.join(REPO_PATH,
                                         "data/ui/coming.gif")) # QMovie存放动态视频，一般配合QLabel使用，可用来存放GIF动态图
        ...
        gif_files = glob.glob(os.path.join(REPO_PATH,"data/ui/normal", '*.gif'), recursive=True)
    ...
    def random_act(self):
        ...
        if not self.action_condition:                               # 正常待机状态
            ...
            self.movie = QMovie(os.path.join(REPO_PATH,"data/ui/click.gif"))           # 读取特殊状态图片路径
    ...
    def get_todo(self):
        """get todo list"""
        todo_str = read_text_from_file(os.path.join(REPO_PATH,
                                                          "data/tools/content/todo_list.txt"))
    ...
    def random_text(self):
        ...
        self.get_todo()
        show_text = random.choice(self.todo_list + [timestr]) if self.todo_list else timestr
    ...
    def random_position(self):
        ...
        screen_geo = QDesktopWidget().screenGeometry()  # 返回当前屏幕的几何信息，包括屏幕的位置、宽度和高度
        pet_geo = self.geometry()                       # 获取部件在其父窗口坐标系统中的几何信息，包括部件的位置和大小
    ...
    def contextMenuEvent(self, event):
        ...
        if action == hide_or_exist:
            if self.windowopacity > 0:
                ...
                self.movie = QMovie(os.path.join(REPO_PATH,"data/ui/coming.gif"))
                self.movie.setScaledSize(QSize(500, 500))
                self.image.setMovie(self.movie)
                self.movie.start()

        if action == chat:
            self.client = ReActClient() if DEFAULT_CLIENT_TYPE == "react" else ChatClient()
            self.client.show()

        if action == quitAction:
            self.movie = QMovie(os.path.join(REPO_PATH,"data/ui/bye.gif"))
            self.movie.setScaledSize(QSize(500, 500))
            self.image.setMovie(self.movie)
            self.movie.start()       
        ...
        if action == feedAction:
            self.movie = QMovie(os.path.join(REPO_PATH,"data/ui/eat.gif"))
            self.movie.setScaledSize(QSize(500, 500))
            self.image.setMovie(self.movie)
            self.movie.start()
