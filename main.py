...
class DesktopPet(QWidget):
    ...
    def init_pet(self):
        ...
        self.movie = QMovie(
            os.path.join(REPO_PATH, "data/ui/coming.gif")
        ...
        gif_files = glob.glob(
            os.path.join(REPO_PATH, "data/ui/normal", "*.gif"), recursive=True
        )
    ...
    def random_act(self):
        ...
        if not self.action_condition:  # 正常待机状态
            ...
            self.movie = QMovie(
                os.path.join(REPO_PATH, "data/ui/click.gif")
            )  # 读取特殊状态图片路径
    ...
    def get_todo(self):
        ...
        todo_str = read_text_from_file(
            os.path.join(REPO_PATH, "data/tools/content/todo_list.txt")
        )
    ...
    def contextMenuEvent(self, event):
        ...
        if action == chat:
            self.client = (
                ReActClient() if DEFAULT_CLIENT_TYPE == "react" else ChatClient()
            )
            self.client.show()
